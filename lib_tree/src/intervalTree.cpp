...
void bode::IntervalTree::i_density(bode::IntervalNode *n,int left,int right,int *dest) {
  if (n == NULL) {
    return;
  } else if (left >= n->r()) {
    i_density(n->getRight(),left,right,dest);
  } else if (right < n->l()) {
    i_density(n->getLeft(),left,right,dest);
  } else {
    i_density(n->getLeft(),left,right,dest);
    
    int start = std::max(n->l()-left,0);
    int end = std::min(n->r()-left,right-left);
//    fprintf(stderr,"left=%d right=%d nleft=%d nright=%d start=%d end=%d\n",left,right,n->l(),n->r(),start,end);
    int c = n->getCount();
...
void bode::IntervalTree::density(int left,int right,int *dest) {
  i_density(root,left,right,dest);
}
