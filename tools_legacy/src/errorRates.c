...
#define SLACK 5 /* extra nucleotides to align against */
...
void tabulate(nucp n,int offset) {
  float delta;
  n[offset].count++;
}

int countMisMatches(char *s1,char *s2) {
  int slen,i,mm;
  slen = strlen(s1);
  mm = 0;
  for (i=0;i<slen;i++) {
...
int isEdge(char *seq,int start,int end,int length) {
  return ((seq[0] == '-' && start <= 1) ||
      (seq[strlen(seq)-1] == '-' && end >= length - 2));
}

char tb1[6000],tb2[6000];
void align(char *read,char *targetf,char *targetr,counterp counts,int* posErrors,int *okay,int *skipped) {
  int mm,tlen;
  char *revstr;
  char *target;
  int score;
  ...
  int scoref,scorer,genomeOffset,readStart,alignLen,i,readPosn;
/*  fprintf(stderr,"lengths: read=%d targ=%d\n",strlen(read),strlen(target));*/
/*  fflush(stderr); */
/*  fprintf(stderr,"source:\n         %s\n    %s\n",read,target);*/
  tlen = strlen(targetf);
  scoref = calignPartialFree(read,targetf);
  if (scoref < 60) {
    scorer = calignPartialFree(read,targetr);
    if (scoref > scorer) {
      score = calignPartialFree(read,targetf);
      target = targetf;
    } else {
      score = scorer;
      target = targetr;
  ...
  if (mm > 10 || isEdge(tb2,calignPosition2start(),calignPosition2end(),tlen)) {
    (*skipped)++;
    fprintf(stdout,"skip mm=%d st=%d en=%d sf=%d sr=%d seq=%s\n",mm,calignPosition2start(),calignPosition2end(),scoref,scorer,read);
    return;
  }
  ...
  for (i=0;i<alignLen;i++) {
    if (tb2[i] == '-') {
      switch (tb1[i]) {
        case 'A':
        case 'a':
          tabulate(counts->ia,genomeOffset);
          readPosn++;
          break;
        case 'C':
        case 'c':
          tabulate(counts->ic,genomeOffset);
          readPosn++;
          break;
        case 'G':
        case 'g':
          tabulate(counts->ig,genomeOffset);
          readPosn++;
          break;
        case 'T':
        case 't':
          tabulate(counts->it,genomeOffset);
          readPosn++;
          break;
        default:
          tabulate(counts->in,genomeOffset);
          readPosn++;
          break;
      }
    } else {
      ...
      switch (tb1[i]) {
        case 'A':
        case 'a':
          tabulate(counts->a,genomeOffset++);
          readPosn++;
/*          counts->acount[genomeOffset++]++; */
          break;
        case 'C':
        case 'c':
          tabulate(counts->c,genomeOffset++);
          readPosn++;
/*          counts->ccount[genomeOffset++]++; */
          break;
        case 'G':
        case 'g':
          tabulate(counts->g,genomeOffset++);
          readPosn++;
/*          counts->gcount[genomeOffset++]++; */
          break;
        case 'T':
        case 't':
          tabulate(counts->t,genomeOffset++);
          readPosn++;
/*          counts->tcount[genomeOffset++]++; */
        ...
        case '-':
          ...
          break;
        default:
          tabulate(counts->n,genomeOffset++);
          readPosn++;
      }
    }
/*
...
void processSequences(FILE *reads,sequence phix) {
/*  int *acount,*ccount,*gcount,*tcount,*ncount,*icount,*dcount; */
  ...
  int realstart;
  fastap seq;
  char *chunkfwd,*chunkrev;
  int qual[100];
  char *phixSeq;
  int phixlen;
  int positionErrors[100],i;
  int skipped = 0;

  seq = fa_alloc(100);
  chunk = seq_alloc();
  counter = allocateCounter(phix->length+2);
  calignInit(6000,2,-2,-6,-1);
  okay = 0;
  missing = 0;
  for (i=0;i<100;i++) {
    positionErrors[i] = 0;
  ...
  chunkfwd = seq_string(phix);
  chunkrev = seq_string(phix);
  reverseComplementInPlace(chunkrev);
  while (fa_next(seq,reads)) {
    align(seq->data,chunkfwd,chunkrev,counter,positionErrors,&okay,&skipped); 
    if (okay % 10 == 0) {
      fprintf(stderr,"%d\r",okay+missing);
    }
  }
  fprintf(stdout,"%d okay, %d skipped\n",okay,skipped);
  fprintf(stdout,"Errors by position:\n");
  for (i=0;i<45;i++) {
    fprintf(stdout,"%5d  %9d\n",i+1,positionErrors[i]);
  }
/*
